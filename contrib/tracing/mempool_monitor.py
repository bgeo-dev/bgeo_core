...
""" Example logging Bgeo Core mempool events using the mempool:added,
...
def main(bgeod_path):
    bgeod_with_usdts = USDT(path=str(bgeod_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    bgeod_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    bgeod_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    bgeod_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    bgeod_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[bgeod_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/bgeod")
        exit(1)
