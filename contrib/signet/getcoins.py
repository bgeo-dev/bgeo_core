...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bgeo-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bgeo-cli', help='bgeo-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Bgeo address to which the faucet should send')
...
parser.add_argument('bgeo_cli_args', nargs='*', help='Arguments to pass on to bgeo-cli (default: -signet)')

args = parser.parse_args()

if args.bgeo_cli_args == []:
    args.bgeo_cli_args = ['-signet']


def bgeo_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bgeo_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bgeo_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bgeo_cli(['getnewaddress', 'faucet', 'bech32'])
