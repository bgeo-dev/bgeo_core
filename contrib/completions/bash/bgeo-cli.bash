# bash programmable completion for bgeo-cli(1)
# Copyright (c) 2012-2022 The Bitcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $bgeo-cli for RPC
_bgeo_rpc() {
...
_bgeo_cli() {
    ...
    # save and use original argument to invoke bgeo-cli for -help, help and RPC
    # as bgeo-cli might not be in $PATH
    bgeo_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($bgeo_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_bgeo_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
