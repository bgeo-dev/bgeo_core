...
const char * const BGEO_CONF_FILENAME = "bgeo.conf";
const char * const BGEO_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bgeo-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", BGEO_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Bgeo
    //   new: C:\Users\Username\AppData\Local\Bgeo
    // macOS: ~/Library/Application Support/Bgeo
    // Unix-like: ~/.bgeo
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Bgeo";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Bgeo";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Bgeo";
#else
    // Unix-like
    return pathRet / ".bgeo";
#endif
