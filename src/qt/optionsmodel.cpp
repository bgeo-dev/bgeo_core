...
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // in later releases by https://github.com/bgeo/bgeo/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because bgeo 22.x will not try to read these.
...
//! Convert enabled/size values to bgeo -prune setting.
...
//! Get pruning enabled value to show in GUI from bgeo -prune setting.
...
//! Get pruning size value to show in GUI from bgeo -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayBgeoUnit")) {
        settings.setValue("DisplayBgeoUnit", QVariant::fromValue(BgeoUnit::BGEO));
    }
    QVariant unit = settings.value("DisplayBgeoUnit");
    if (unit.canConvert<BgeoUnit>()) {
        m_display_bgeo_unit = unit.value<BgeoUnit>();
    } else {
        m_display_bgeo_unit = BgeoUnit::BGEO;
        settings.setValue("DisplayBgeoUnit", QVariant::fromValue(m_display_bgeo_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if bgeo.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_bgeo_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BgeoUnit>() == m_display_bgeo_unit) return;
    m_display_bgeo_unit = new_unit.value<BgeoUnit>();
    QSettings settings;
    settings.setValue("DisplayBgeoUnit", QVariant::fromValue(m_display_bgeo_unit));
    Q_EMIT displayUnitChanged(m_display_bgeo_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/bgeo/bgeo/pull/8273
    ...
    // (https://github.com/bgeo-core/gui/issues/567).
