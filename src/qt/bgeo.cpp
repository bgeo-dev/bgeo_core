...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<BgeoUnit>("BgeoUnit");
#else
    qRegisterMetaType<BgeoUnit>("BgeoUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bgeo_de.qm (shortcut "de" needs to be defined in bgeo.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bgeo_de_DE.qm (shortcut "de_DE" needs to be defined in bgeo.qrc)
...
static const char* qt_argv = "bgeo-qt";

BgeoApplication::BgeoApplication()
...
void BgeoApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BgeoApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BgeoGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BgeoApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BgeoApplication::createOptionsModel(bool resetSettings)
{
...
void BgeoApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BgeoGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BgeoGUI::quitRequested, this, &BgeoApplication::requestShutdown);
...
void BgeoApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BgeoApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BgeoApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BgeoApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BgeoApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BgeoApplication::handleRunawayException);
    connect(this, &BgeoApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BgeoApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BgeoApplication::parameterSetup()
{
...
void BgeoApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BgeoApplication::requestInitialize()
{
...
void BgeoApplication::requestShutdown()
{
...
void BgeoApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // bgeo: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BgeoGUI::handlePaymentRequest);
            connect(window, &BgeoGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BgeoApplication::handleRunawayException(const QString &message)
{
...
void BgeoApplication::handleNonFatalException(const QString& message)
{
...
WId BgeoApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BgeoApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BgeoGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(bgeo);
    Q_INIT_RESOURCE(bgeo_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(BGEO_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see bgeo-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, PACKAGE_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see bgeo-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse bgeo.conf, determine network, switch to network specific
    ...
    // bgeo: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
