...
#include <config/bgeo-config.h> // IWYU pragma: keep

#include <qt/bgeogui.h>
...
const std::string BgeoGUI::DEFAULT_UIPLATFORM =
...
BgeoGUI::BgeoGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &BgeoGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &BgeoGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &BgeoGUI::showModalOverlay);
...
void BgeoGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Bgeo address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and bgeo: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &BgeoGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &BgeoGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &BgeoGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Bgeo addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Bgeo addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Bgeo Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Bgeo Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a bgeo: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Bgeo command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &BgeoGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &BgeoGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &BgeoGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &BgeoGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &BgeoGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &BgeoGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &BgeoGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &BgeoGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &BgeoGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &BgeoGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &BgeoGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &BgeoGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &BgeoGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &BgeoGUI::showDebugWindow);
}

void BgeoGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void BgeoGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &BgeoGUI::setCurrentWalletBySelectorIndex);
...
void BgeoGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &BgeoGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &BgeoGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &BgeoGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &BgeoGUI::showProgress);
...
#ifdef ENABLE_WALLET
void BgeoGUI::enableHistoryAction(bool privacy)
{
...
void BgeoGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &BgeoGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &BgeoGUI::removeWallet);
...
WalletController* BgeoGUI::getWalletController()
{
    return m_wallet_controller;
}

void BgeoGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &BgeoGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &BgeoGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &BgeoGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &BgeoGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &BgeoGUI::incomingTransaction);
    connect(this, &BgeoGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void BgeoGUI::removeWallet(WalletModel* walletModel)
{
...
void BgeoGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void BgeoGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void BgeoGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BgeoGUI::setWalletActionsEnabled(bool enabled)
{
...
void BgeoGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &BgeoGUI::toggleHidden);
...
void BgeoGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void BgeoGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void BgeoGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void BgeoGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void BgeoGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void BgeoGUI::openClicked()
{
...
void BgeoGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BgeoGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BgeoGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BgeoGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BgeoGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BgeoGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void BgeoGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void BgeoGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Bgeo network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void BgeoGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void BgeoGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void BgeoGUI::updateHeadersSyncProgressLabel()
{
...
void BgeoGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void BgeoGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &BgeoGUI::quitRequested);
...
void BgeoGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void BgeoGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &BgeoGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void BgeoGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void BgeoGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &BgeoGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &BgeoGUI::show);
                e->ignore();
            }
        }
    }
...
void BgeoGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BgeoGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BgeoGUI::incomingTransaction(const QString& date, BgeoUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(BgeoUnits::formatWithUnit(unit, amount, true));
...
void BgeoGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BgeoGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BgeoGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BgeoGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BgeoGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void BgeoGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void BgeoGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void BgeoGUI::updateWindowTitle()
{
...
void BgeoGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BgeoGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BgeoGUI::detectShutdown()
{
...
void BgeoGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void BgeoGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(BgeoGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = BgeoGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void BgeoGUI::subscribeToCoreSignals()
{
...
void BgeoGUI::unsubscribeFromCoreSignals()
{
...
bool BgeoGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<BgeoUnit> units = BgeoUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const BgeoUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, BgeoUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const BgeoUnit u : BgeoUnits::availableUnits()) {
        menu->addAction(BgeoUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(BgeoUnit newUnits)
{
    setText(BgeoUnits::longName(newUnits));
}
