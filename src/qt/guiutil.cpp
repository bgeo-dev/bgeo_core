...
#include <qt/bgeoaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Bgeo address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BgeoAddressEntryValidator(parent));
    widget->setCheckValidator(new BgeoAddressCheckValidator(parent));
}
...
bool parseBgeoURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no bgeo: URI
    if(!uri.isValid() || uri.scheme() != QString("bgeo"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!BgeoUnits::parse(BgeoUnit::BGEO, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseBgeoURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBgeoURI(uriInstance, out);
}

QString formatBgeoURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("bgeo:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BgeoUnits::format(BgeoUnit::BGEO, info.amount, false, BgeoUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openBgeoConf()
{
    ...
    /* Open bgeo.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Bgeo.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Bgeo (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Bgeo (%s).lnk", ChainTypeToString(chain)));
}
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "bgeo.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("bgeo-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a bgeo.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Bgeo\n";
        else
            optionFile << strprintf("Name=Bgeo (%s)\n", ChainTypeToString(chain));
